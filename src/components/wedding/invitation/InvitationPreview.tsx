  ...
  return (
    ...
    >
      ...
      <div className="relative h-full flex flex-col items-center pt-8 pb-6 px-4 z-10 text-center font-sans">
        ...
        {/* ================= COUPLE SECTION (MATCHES IMAGE) ================= */}
        <div className="flex-1 w-full flex flex-col justify-center items-center py-2 space-y-8">

          {/* Groom Row */}
          <div className="flex items-center w-full px-8">
            <Avatar
              className={cn(
                "w-28 h-28 border-[3px] shadow-xl",
                isLightTemplate
          ...
          <div className="flex items-center justify-center opacity-60">
            <div
              className={cn(
                "h-px w-20 bg-current",
                config.accentColor
              )}
            ></div>
            <span
              className={cn(
                "font-serif italic text-2xl mx-4",
                config.accentColor
              )}
            >
              &
          ...
          {/* Bride Row */}
          <div className="flex items-center w-full px-8 justify-end">
        ...
        <div className="flex-none pt-2 pb-4 w-full space-y-6">
          ...
          <div className="space-y-1">
            ...
            <h3 className="text-sm font-bold tracking-wide uppercase px-4 leading-relaxed font-serif text-gray-900 drop-shadow-sm">
              {wedding.venue}
            </h3>
          </div>
        </div>
        ...
        {wedding.events && wedding.events.length > 0 && (
          <div className="w-full px-6 pb-4">
            <div className="grid grid-cols-2 gap-4">
              {wedding.events.slice(0, 6).map((event, index) => (
                <div
                  key={index}
                  className="flex flex-col items-center justify-center py-3 px-2 rounded-xl border border-amber-200 bg-amber-50 shadow-sm"
                >
                  <span className="text-[10px] uppercase font-bold tracking-wider mb-1 text-gray-800">
